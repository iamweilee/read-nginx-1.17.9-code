...
main(int argc, char *const *argv)
{
    ...
    if (ngx_show_version) {
        ...
        // 如果不需要校验配置文件则直接退出，否则到下面去校验文件在退出
        if (!ngx_test_config) {
            return 0;
        }
    }
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    // 初始化cycle的listening字段，指向可以保存10个ngx_listening_t结构体的数组首地址，用来保存继承过来的socket信息
    ...
    // 设置标记位，有继承的socket
    ngx_inherited = 1;
    // 根据系统提供的关于socket的能力，初始化listening结构体的一些字段。
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        while (*p) {
            ...
            // 设置路径前缀
            case 'p':
                // *p非空，则说明值是紧跟p的后面，保存下来
                if (*p) {
                    ngx_prefix = p;
                    goto next;
                }
                // *p为空说明是nginx -p xxx这种形式。保存下一个参数的值
                if (argv[++i]) {
...
ngx_process_options(ngx_cycle_t *cycle)
{
    ...
    // 解析出配置文件的目录路径，比如a/b/nginx.conf=>/a/b
